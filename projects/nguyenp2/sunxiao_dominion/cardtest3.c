 ...
 * Date: 4/30/17
 * CS362-400
 * Description: Unit test for the card Council Room
 *****************************************************************/
...
int main() 
{
  ...
  failFlag = assertEqual(testG.numBuys, G.numBuys + 1, failFlag);
  
	// TEST: +1 card to other player's hand
	printf("\nTEST: +1 card to player1's hand\n");
  
  otherPlayerNewCards = 1;

	printf("player1 hand count = %d, expected = %d\n", testG.handCount[otherPlayer], 
      G.handCount[otherPlayer] + otherPlayerNewCards);
	failFlag = assertEqual(testG.handCount[otherPlayer], 
      G.handCount[otherPlayer] + otherPlayerNewCards, failFlag);

  printf("player1 deck count = %d, expected = %d\n", testG.deckCount[otherPlayer],
      G.deckCount[otherPlayer] - otherPlayerNewCards);
	failFlag = assertEqual(testG.deckCount[otherPlayer], 
      G.deckCount[otherPlayer] - otherPlayerNewCards, failFlag);
